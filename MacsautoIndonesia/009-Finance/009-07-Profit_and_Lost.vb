Public Class _009_07_Profit_and_Lost
    ...
    Public Sub getData()
        ...
        If zero.Checked = True Then
            query = "SELECT b.GLNUM,D.ACTDS,GLDES,(SUM(CASE WHEN PSTKY = '20' THEN PSAMT ELSE '0' END)- SUM(CASE WHEN PSTKY = '10' THEN PSAMT ELSE '0' END))'AMOUNT' FROM JOURHD A, JOURDT B,GLACCOUNTMS C, ACCOUNTTYPE D  WHERE A.DOCID = B.DOCID AND C.GLNUM =B.GLNUM AND C.ACTID = D.ACTID and isPnl = 'X' and DSTAT <> 'X' AND PSTDT LIKE '" + ComboBox1.SelectedItem.ToString + "%' AND DTNUM <> 'CL' GROUP BY  B.GLNUM"
        Else
