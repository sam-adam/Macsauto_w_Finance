    ...
    Private Sub TryPostJournal(ByVal transactionId As String)
        ...
        If ServiceGrid.Rows.Count > 0 Then
            ...
            While reader.Read()
                ...
                Dim row As DataGridViewRow = ServiceGrid.Rows.OfType(Of DataGridViewRow).First(Function() row.Cells(1).Value.ToString() = reader("idsvc").ToString())
        ...
        If ProductGrid.Rows.Count > 0 Then
            ...
            While reader.Read()
                Dim dictionary As New Dictionary(Of String, String)
                Dim row As DataGridViewRow = ProductGrid.Rows.OfType(Of DataGridViewRow).First(Function() row.Cells(1).Value.ToString() = reader("idsvc").ToString())
                Dim subTotal As Double = Double.Parse(row.Cells(3).Value) * Double.Parse(row.Cells(5).Value)
        ...
        If glAccounts.Count = (ServiceGrid.Rows.Count + ProductGrid.Rows.Count) Then
            DoInTransaction(
                Function()
                    ExecQueryNonReader("INSERT INTO jourhd (SELECT (COUNT(*) + 1), NOW(), NOW(), '" & transactionId & "', 'AUTOPOST TRANSAKSI', '', '" & LoggedInEmployee.Id & "', 'TR', '' FROM jourhd)")
                    ...
                    If (jourReader.Read()) Then
                        ...
                        If PayMethod.SelectedItem.ToString() = "Cash" Then
                            ExecQueryNonReader("INSERT INTO jourdt VALUES('" & newJournalId & "', '1000.01', '10', '" & TotalPayment.Text & "', '')")
                        Else
                            ExecQueryNonReader("INSERT INTO jourdt VALUES('" & newJournalId & "', '1000.02', '10', '" & TotalPayment.Text & "', '')")
                        End If
